...
# Asynchronously extract invoice data from the text
async def extract_invoice_data(markdown_text):
    ...
    extracted_data = tokenizer.decode(output[0], skip_special_tokens=True).split("}")[1] # Fixed splitting error
...
# Separate currency from a value
...
# Asynchronously process remaining PDFs
...
# Upload screen for uploading PDFs
def upload_screen():
...
# Review screen for reviewing extracted data
def review_screen():
